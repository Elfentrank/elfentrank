...
import time, json, threading, os, random, httpx
import tls_client
...
def boost_server(invite:str , months:int, token:str, thread:int, nick: str):
    ...
    #ja3_string = fingerprints[random.randint(0, (len(fingerprints)-1))]['ja3'], client_identifier = random.choice(client_identifiers)
    try:
        session = tls_client.Session(ja3_string = None, client_identifier = random.choice(client_identifiers))
...
def menu():
    os.system('title discord.gg/codeproducts')
...
\  \__(  <_> ) /_/ \  ___/      / /_/  > /_/  >
 \___  >____/\____ |\___  > /\  \___  /\___  / 
     \/           \/    \/  \/ /_____//_____/  
                ...
                Made by: code.gg
''')
    invite = input("Invite: discord.gg/")
    if ".gg/" in invite:
    ...
    print()
    print(f"\n{Style.BRIGHT}{Fore.CYAN}Time Taken: {time_went} seconds\nSuccessful Boosts: {len(variables.success_tokens)*2}\n")
